...
export class CommentService {
  ...
  recentComments() {
    return this.apollo.query({ query: RECENT_COMMENTS_QUERY, variables: { limit: 8 } }).pipe(
      map((res) => this.responseService.formatResponseObjects(res)),
      map((result: any) => result.data.recentComments)
    );
  }
  ...
  removeComment(commentId: string): Observable<Comment> {
    return this.apollo
      ...
      .pipe(
        map((res) => this.responseService.formatResponseObjects(res)),
        map((result: any) => result.data.deleteComment)
      );
  }
  ...
  editComment(commentId: string, body: string): Observable<Comment> {
    return this.apollo
      ...
      .pipe(
        map((res) => this.responseService.formatResponseObjects(res)),
        map((result: any) => result.data.updateComment)
      );
  }
}
